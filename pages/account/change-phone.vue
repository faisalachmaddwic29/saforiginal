<template>
  <div>
    <form @submit="onSubmitPhone" class="relative pb-[60px]" v-if="showFormPhone">
        ...
        <div class="flex flex-col gap-4">
          <div class="flex flex-col gap-2">
            ...
            <FormInput
              ...
              v-model="new_phone"
              :error="phoneForm.errors.value.new_phone"
            />
          </div>
					<TitleInfo type="warning">Gunakan nomor yang terdaftar pada akun whatsapp anda untuk melakukan verifikasi nomor</TitleInfo>
        </div>
      </div>
    ...
    <form @submit="onSubmitOtp" class="relative pb-[60px]" v-if="showFormOtp">
      <div class="px-4 pt-5 pb-5 w-full">
        <div class="flex flex-col gap-6">
					<div class="text-title text-sm">Silahkan masukkan kode OTP yang telah kami kirimkan ke <strong>{{new_phone}}</strong></div>
          <div class="flex flex-col gap-2">
...
const onSubmitPhone = phoneForm.handleSubmit(async (values) => {
	try {
		...
		const response = await apiService.post('/auth/phone/change/request', {
...
const onSubmitOtp = otpForm.handleSubmit(async (values) => {
	try {
		...
		const response = await apiService.post('/auth/phone/change/confirm', {
			otp: values.otp,
