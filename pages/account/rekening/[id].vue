...
const onSubmit = handleSubmit(async (values) => {
	...
	try {
		const response = await apiService.put('/v1/bank-accounts/' + route.params.id,{
...
const getBanksOption = async () => {
  ...
  try {
    const response = await apiService.get("/v1/banks");
