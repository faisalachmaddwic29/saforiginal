<template>
  <div>
    <form @submit="onSubmitEmail" class="relative pb-[60px]" v-if="showFormEmail">
        ...
        <div class="flex flex-col gap-4">
          <div class="flex flex-col gap-2">
            ...
            <FormInput
              ...
              v-model="new_email"
              :error="emaiForm.errors.value.new_email"
            />
          </div>
        </div>
      </div>
    ...
    <form @submit="onSubmitOtp" class="relative pb-[60px]" v-if="showFormOtp">
      <div class="px-4 pt-5 pb-5 w-full">
        <div class="flex flex-col gap-6">
					<div class="text-title text-sm">Silahkan masukkan kode OTP yang telah kami kirimkan ke <strong>{{new_email}}</strong></div>
          <div class="flex flex-col gap-2">
            <FormOtp v-model="otp" :fields="6" :isError="Boolean(otpForm.errors.value.otp)" />

						<FormMessageError v-if="otpForm.errors.value.otp" :message="otpForm.errors.value.otp" class="justify-center" />
          </div>
					<p class="text-xs md:text-sm text-[#1E293B] dark:text-[#94A3B8]">Tidak menerima kode verifikasi? <span @click="onSubmitEmail" class="cursor-pointer text-primary font-bold pl-0.5">Kirim ulang</span></p>
        </div>
      </div>
      <div class="fixed w-full z-10 bottom-0 left-0 bg-[#F9F9F9] dark:bg-[#0F172A] shadow-[0px_-2px_4px_rgba(0,0,0,0.05)] p-4">
        <AppContainer>
		...
		<div class="relative pb-[60px]" v-if="showFormSuccess">
      <div class="px-4 pt-5 pb-5 w-full">
      ...
      <div class="fixed w-full z-10 bottom-0 left-0 bg-[#F9F9F9] dark:bg-[#0F172A] shadow-[0px_-2px_4px_rgba(0,0,0,0.05)] p-4">
        <AppContainer>
          <Button class="w-full" @click="router.back()">
            Oke
          </Button>
        </AppContainer>
      </div>
...
const onSubmitEmail = emaiForm.handleSubmit(async (values) => {
	try {
		...
		const response = await apiService.post('/auth/email/change/request', {
...
const onSubmitOtp = otpForm.handleSubmit(async (values) => {
	try {
		...
		const response = await apiService.post('/auth/email/change/confirm', {
			otp: values.otp,
