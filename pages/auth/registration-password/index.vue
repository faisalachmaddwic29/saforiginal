<template>
	<form @submit="onSubmit" class="flex flex-col min-dvh-screen px-4 py-5 relative">
		...
		<div class="flex flex-col gap-5 mb-2.5">
            ...
            <ComboboxGroup class="h-60 overflow-y-scroll">
              ...
              >
				...
				<p v-if="errors.address && addressTouched" class="mt-1 text-sm text-red-600 flex gap-1 items-center">
					<Icon name="ion:alert-circle-outline" class="text-lg text-red-600" />
					{{ capitalize(errors.address) }}
				</p>
			</div>

			<TitleInfo>Pastikan data yang diisi sesuai, e-tiket akan di kirimkan ke alamat email dan nomor telepon yang dicantumkan</TitleInfo>
			...
			<div class="gender">
				...
				<p v-if="errors.gender && genderTouched" class="mt-1 text-sm text-red-600 flex gap-1 items-center">
					<Icon name="ion:alert-circle-outline" class="text-lg text-red-600" />
		...
		<p class="text-xs md:text-sm text-[#1E293B] dark:text-[#94A3B8]">Sudah punya akun? <NuxtLink to="/auth/login-password" class="text-primary font-bold pl-0.5">Masuk disini</NuxtLink></p>
