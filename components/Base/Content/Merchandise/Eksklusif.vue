  ...
  <div class="flex flex-col gap-5 px-4 py-1 pb-4">
    ...
    <div class="mt-2">
      <div class="flex items-center flex-1 rounded-full border border-[rgba(151,151,151,0.2)] text-[rgba(77,77,77,0.03)] h-[48px] px-4 shadow-md">
        ...
        <input v-model="search" type="text" placeholder="Cari Baju, Kain, Peci . . ." class="font-manrope bg-transparent placeholder-menu/30 text-menu text-base focus:outline-none w-full" />
      </div>
    </div>
    ...
    <div class="w-full h-[100px] relative rounded-2xl overflow-hidden p-4">
      ...
      <div class="w-full flex items-center justify-between gap-4">
        ...
        <div class="aspect-auto flex-shrink-0">
          <NuxtImg src="/images/merchandise/logo.png" alt="logos" class="w-fit h-[70px] ml-auto" />
        </div>
      </div>
    </div>
    ...
    <div class="w-full">
      <div v-if="merchandises.length" class="w-full mx-auto">
        <div class="grid grid-cols-2 gap-4">
          <CardMerchandise v-for="item in merchandises" :key="item.id + '-merchandise'" :product="item" @click="$router.push(`/merchandise/ekslusif/${item.slug}`)" />
        </div>
      ...
      <!-- Empty state -->
      <div v-else-if="!isFirstLoading && merchandises.length === 0" class="text-center py-8">
...
// State
const merchandises = ref<Product[]>([]);
const page = ref(1);
...
const buildParams = ({
  ...
  params.append('type', ProductType.MERCHANDISE);
...
const fetchMerchandises = async () => {
  ...
  try {
    ...
    if (data.products && data.products.length > 0) {
      merchandises.value.push(...data.products);
