<template>
  <!--  -->
  <div
    class="group w-[180px] md:w-[210px] overflow-hidden cursor-pointer transition duration-200 bg-white text-[#1E293B] dark:bg-[#0F172A] dark:text-white flex-shrink-0"
    @click="handleClick"
  >
    <div class="relative overflow-hidden rounded-lg aspect-[3/3.5]">
      <NuxtImg
        ...
        class="aspect-[3/4] w-full h-full object-cover transition-transform duration-500 ease-in-out group-hover:scale-110"
      />
    </div>
    <div class="w-full pt-0.5">
      ...
      <div
        v-if="props.rating"
        class="w-max flex items-center text-xs gap-[3px] mt-0.5 mb-1 md:mb-2 md:mt-1 bg-rating/10 px-1 py-0.5 rounded border border-rating"
      >
        <Icon
          name="material-symbols:star-rounded"
          class="w-4 h-4 text-lg text-rating"
        />
