<template>
  ...
  <div class="group cursor-pointer w-full bg-white rounded-sm border border-[rgba(148,163,184,0.2)] text-[#1E293B] dark:bg-[#0F172A] dark:text-white overflow-hidden flex-shrink-0">
    <div class="relative border-b border-[rgba(148,163,184,0.2)] overflow-hidden aspect-[1/1]">
      <NuxtImg :src="props?.thumbnail" :alt="props?.id" class="aspect-[1/1] w-full h-full object-fill transition-transform duration-500 ease-in-out group-hover:scale-110 rounded-t-xs" />
    </div>
    <div class="p-3 md:p-4">
      ...
      <p class="text-base md:text-lg font-bold text-primary">{{ formatPrice(price) }}</p>
    </div>
  </div>
</template>
...
const props = defineProps({
  thumbnail: {
    type: String,
    default: '/images/merchandise/background.png',
  },
  id: {
    type: String,
    default: '1',
  ...
  title: {
    type: String,
    default: 'Buku Lapis Lapis Keberkahan bersama teman teman',
  },
  rating: {
    type: Number,
    default: 4.8,
